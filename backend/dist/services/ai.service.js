...
const client = env_1.env.OPENAI_API_KEY ? new openai_1.default({ apiKey: env_1.env.OPENAI_API_KEY }) : null;
const analyzeEmotion = async (inputText) => {
    ...
    const response = await client.responses.create({
        ...
        input: [
            {
                role: "system",
                content: "You are an emotional wellness classifier. Respond ONLY JSON with fields: emotionalState, positivityScore, suggestions. emotionalState must be one of Anxiety, Stress, Depression, Positive, Hopeful, Lonely, Fearful. positivityScore is integer 0-100. suggestions is array of 3 short healing suggestions."
            },
            {
                role: "user",
                content: inputText
